...
// GET /laws - List all laws with pagination and search
router.get('/', async (req, res) => {
  try {
    ...
    if (q) {
      query = `
        SELECT id, issuing_authority, number, year, title, issue_date, publish_date,
               LEFT(preamble, 200) as preamble_preview
        FROM laws
    ...
    console.error('[Laws] Error:', err.message);
